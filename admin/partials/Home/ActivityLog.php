        ...
        for (i = 0; i < diffs.length; i++) {
            ...
            for (let j = 0; j < linesLeft.length; j++) {
                if (diffs[i]['path1']['line'] == j + 1 && diffs[i]['type'] == 'eq' || diffs[i]['type'] == 'type') {
                    $(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j], `<span style="color:#000; text-decoration: line-through;background-color:#FFBF86">${linesLeft[j]}</span>`));
                } else if (diffs[i]['path1']['line'] == j + 1 && diffs[i]['type'] == 'right') {
                    $(`#${blockLeftId}`).text(textLeft.replace(linesLeft[j], `<span style="color:#27ae60;">${linesLeft[j]}</span>`));
                }
            }
            ...
            for (let j = 0; j < linesRight.length; j++) {
                if (diffs[i]['path2']['line'] == j + 1 && diffs[i]['type'] == 'eq') {
                    $(`#${blockRightId}`).text(textRight.replace(linesRight[j], `<span style="color:#000;background-color:#ACEAC6"">${linesRight[j]}</span>`));
                }
